...
char *labels[] = {"airplane","automobile","bird","cat","deer","dog","frog","horse","ship","truck"};
    ...
    for(i = 0; i < train.X.rows; ++i){
        ...
        int class_ = max_index(train.y.vals[i], 10);
        char buff[256];
        sprintf(buff, "data/cifar/train/%d_%s",i,labels[class_]);
    ...
    for(i = 0; i < test.X.rows; ++i){
        ...
        int class_ = max_index(test.y.vals[i], 10);
        char buff[256];
        sprintf(buff, "data/cifar/test/%d_%s",i,labels[class_]);
        save_image_png(im, buff);
    }
}
