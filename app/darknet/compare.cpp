...
int bbox_comparator(const void *a, const void *b)
{
    ...
    free_image(im2);
    free(X);
    if (predictions[class_*2] > predictions[class_*2+1]){
        return 1;
    }
    return -1;
}

void bbox_update(sortable_bbox *a, sortable_bbox *b, int class_, int result)
{
    int k = 32;
    float EA = 1./(1+pow(10, (b->elos[class_] - a->elos[class_])/400.));
    float EB = 1./(1+pow(10, (a->elos[class_] - b->elos[class_])/400.));
    float SA = result ? 1 : 0;
    float SB = result ? 0 : 1;
    a->elos[class_] += k*(SA - EA);
    b->elos[class_] += k*(SB - EB);
}

void bbox_fight(network net, sortable_bbox *a, sortable_bbox *b, int classes, int class_)
{
...
void BattleRoyaleWithCheese(char *filename, char *weightfile)
{
    ...
    for (class_ = 0; class_ < classes; ++class_){
        ...
        for(round = 1; round <= 100; ++round){
            ...
            for(i = 0; i < N/2; ++i){
                bbox_fight(net, boxes+i*2, boxes+i*2+1, classes, class_);
            }
            qsort(boxes, N, sizeof(sortable_bbox), elo_comparator);
        ...
        char buff[256];
        sprintf(buff, "results/battle_%d.log", class_);
        FILE *outfp = fopen(buff, "w");
        for(i = 0; i < N; ++i){
            fprintf(outfp, "%s %f\n", boxes[i].filename, boxes[i].elos[class_]);
        }
        fclose(outfp);
    }
