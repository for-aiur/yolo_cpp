...
void delta_region_class(float *output, float *delta, int index, int class_, int classes, tree *hier, float scale, float *avg_cat)
{
    int i, n;
    if(hier){
        float pred = 1;
        while(class_ >= 0){
            pred *= output[index + class_];
            int g = hier->group[class_];
            int offset = hier->group_offset[g];
            for(i = 0; i < hier->group_size[g]; ++i){
                delta[index + offset + i] = scale * (0 - output[index + offset + i]);
            }
            delta[index + class_] = scale * (1 - output[index + class_]);
        ...
        *avg_cat += pred;
        ...
        for(n = 0; n < classes; ++n){
            delta[index + n] = scale * (((n == class_)?1 : 0) - output[index + n]);
            if(n == class_) *avg_cat += output[index + n];
        }
    }
}
...
void forward_region_layer(const layer l, network_state state)
{
    ...
    std::cout << "memset begins" << std::endl;
    memset(l.delta, 0, l.outputs * l.batch * sizeof(float));
    std::cout << "memset ends" << std::endl;
    float avg_iou = 0;
    float recall = 0;
    float avg_cat = 0;
    float avg_obj = 0;
    ...
    *(l.cost) = 0;
    ...
    std::cout << "for bbbb" << std::endl;
    for (b = 0; b < l.batch; ++b) {
        if(l.softmax_tree){
            int onlyclass = 0;
            std::cout << "for tttt" << std::endl;
            for(t = 0; t < 30; ++t){
                box truth = float_to_box(state.truth + t*5 + b*l.truths);
                if(!truth.x) break;
                int class_ = state.truth[t*5 + b*l.truths + 4];
                float maxp = 0;
                int maxi = 0;
                if(truth.x > 100000 && truth.y > 100000){
                    for(n = 0; n < l.n*l.w*l.h; ++n){
                        ...
                        float p = scale*get_hierarchy_probability(l.output + index, l.softmax_tree, class_);
                        if(p > maxp){
                            maxp = p;
                            maxi = n;
                        }
                    ...
                    delta_region_class(l.output, l.delta, index, class_, l.classes, l.softmax_tree, l.class_scale, &avg_cat);
        ...
        for(t = 0; t < 30; ++t){
            ...
            delta_region_class(l.output, l.delta, best_index + 5, class_, l.classes, l.softmax_tree, l.class_scale, &avg_cat);
            ++count;
            ++class_count;
        }
    }
    std::cout << "for finished" << std::endl;
    //printf("\n");
#ifndef GPU
